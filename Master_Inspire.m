function [Rating, Cache, Bandwidth, Mean, lenD, Rmean, Rvar, DownRate, cP, StarveCnt, runCnt] = Master_Inspire(maxl, Data, ServeTime, Group, c, CacheSize, StartDelay, TimeSlot, W, P, Segmentlen)
...
R(2, 1) = 0.35; R(2, 2) = 0.5; R(2, 3) = 0.75; R(2, 4) = 1.1; R(2, 5) = 1.5; R(2, 6) = 2.5; lenR(2) = 6;
R(3, 1) = 0.3; lenR(3) = 1;
...
while(1)
    ...
    if (mod(CurrentTime, CalcTime) < eps)
        ...
        while(1)
            ...
            [r, r0] = Step2_Inspire(U(1 : p, :), p, LL(1 : q, :), q, Buffer, SerTag, maxl, R, lenR, W, Group, CalcTime, c, P, alpha, StartDelay, Rating(:, nowSl - 1), lack, GetR, Segmentlen, Para); %Step2
            [Pr, u, flag, w] = Step1(r, CalcTime, c, Buffer, SerTag, maxl, W, P, lack, Segmentlen, Para); %Step3
            runCnt(nowSl) = runCnt(nowSl) + 1;
            if (flag == 1)
                if (Pr < Prime + xi || abs(Pr - Prime - eps) < xi)
                    break;
                else
                    p = p + 1;
                    U(p, :) = u;
                    Prime = Pr;
                    %  [r, r0] = Step2(U, p, LL, q, Cache(:, nowSl), Tag, maxl, R, lenR, W * 10, Group, TimeSlot, c, P, alpha);
                end
            else
                q = q + 1;
                %min alpha; sub Ax = b
                f = zeros(1, maxl + 1); A = zeros(maxl + 2, maxl + 1); b = zeros(1, maxl + 2);
                f(maxl + 1) = 1;
                for i = 1 : maxl
                    if (SerTag(i) == 1)
                        A(maxl + 1, i) = 1; A(maxl + 2, i) = c(i);
                        A(i, i) = -1; A(i, maxl + 1) = -1;
                        b(i) = -max(0, r(i) * (1 - (Buffer(i) - Segmentlen(i) * Para) / CalcTime));
                    end
                end
                A(maxl + 1, maxl + 1) = -1; b(maxl + 1) = W;
                A(maxl + 2, maxl + 1) = -1; b(maxl + 2) = P;
                Aeq = []; Beq = []; lb = zeros(maxl + 1, 1);
